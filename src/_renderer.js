...
function displayLine() {
    ...
    switch (true) {
        case i === 2:
            bootScreen.innerHTML += `chloelam Kernel version ${electron.remote.app.getVersion()} boot at ${Date().toString()}; root:xnu-1699.22.73~1/RELEASE_X86_64`;
        case i === 4:
...
async function displayTitleScreen() {
    ...
    bootScreen.innerHTML = "<h1>chloelam</h1>";
...
async function initUI() {
    ...
    require("username")().then(user => {
        ...
        window.audioManager.greeting.play();
    }).catch(() => {
    ...
    window.term[0].term.writeln("\033[1m" + `Welcome to chloelam v${electron.remote.app.getVersion()} - Electron v${process.versions.electron}` + "\033[0m");

    await _delay(100);

    ipc.send('cmatrix',{args:'-a'})
